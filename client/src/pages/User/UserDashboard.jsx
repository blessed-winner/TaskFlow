         ...
         <div className='bg-white p-6 rounded-lg shadow-md'>
          ...
          <div className='flex flex-col gap-4'>
            {totalCount > 0 ? totalTasks.map((task,index)=>(
                ...
                <p className={` px-2 py-1 text-xs rounded-full font-medium ${task.status.toLowerCase() === 'in progress' && ' text-blue-700 bg-blue-100/60'} ${task.status.toLowerCase() === 'pending' && ' text-orange-700 bg-orange-100/60'} ${task.status.toLowerCase() === 'completed' && ' text-green-700 bg-green-100/60'}`}>{task.status.toLowerCase()}</p>
             </div>
            )) : (
