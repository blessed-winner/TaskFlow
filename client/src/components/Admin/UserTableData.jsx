...
const UserTableData = ({user,fetchUsers}) => {
    const { name,email,role,dept } = user
  ...
  return (
    <tr className='border-b border-gray-200 bg-white text-gray-800 hover:bg-blue-100/20 transition-all'>
      <td className='py-3 px-2 xl:px-4 flex items-center gap-3'>
        <h1 className='text-white bg-blue-500 inline-block px-4 py-3 rounded-full'>{name.slice(0,1)}</h1>
      ...
      <td className='py-3 px-2 xl:px-4 font-light max-md:hidden'>{dept}</td>
